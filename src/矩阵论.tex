...
* 线性变换
    ...
    \Example 
        ...
        * 投影变换
            ...
            \Property
                * 对投影的结果再做投影结果不变, 即, P是投影矩阵 $ => $ P是幂等矩阵($P = P^2$)
                * $A = \U [\Sigma & 0\\ 0 & 0] \V^H => A^+ = \V [\Sigma^{-1} & 0\\ 0 & 0] \U^H$
                * $N(A) = R(I - A)$
                    \Proof
                        $
        ...
        * 初等旋转变换
            \def{初等旋转变换}
                $T_{ij} = I + [\mb 0\\ & \cos\theta|_{(i,i)}& 0 & \sin\theta|_{(i,j)}& \\ & 0&0&0\\&-\sin\theta|_{(j,i)}& 0 & \cos\theta|_{(j,j)}\\ &&&&0 \me] - [\mb 0\\ &1|_{(i,i)}\\&&0\\&&& 1 |_{(j,j)}\\&&&&0 \me]$
