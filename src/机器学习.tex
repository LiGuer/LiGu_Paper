...
* 强化学习
	...
	* 探索-利用平衡
		...
		* 上置信界算法
			...
			$
				=> & \P (\mu \ge \bar{\mu} + \sqrt{\frac{2}{n} \ln \frac{1}{\sigma}}  ) \le \sigma
				=> & \P (\bar{\mu} \in  [\mu - \sqrt{\frac{2}{n} \ln \frac{1}{\sigma}}, \mu +  \sqrt{\frac{2}{n} \ln \frac{1}{\sigma}}  ]  ) \ge 1 - 2 \sigma
			$
			}
				
		* Softmax算法
			\bf{soft-max分布}:
				$\operatorname{Pr} \{A_t=a \} = \frac{e^{H_t(a)}}{\sum_{b=1}^{k} e^{H_t(b)}} = \P_{π,t}(a)$
	...
	* 基于价值函数
		...
		* 动态规划
			...
			* 策略迭代 = 策略评估$V_π$ + 策略改进$π \to π'$
				* \bf{策略评估}: 求当前策略下的累计奖励$V_π$.
					解析解: (计算复杂度$|S|^3$)
					$
						V_π(s)
						&=\E_π [R_{t+1}+\gamma V_π (S_{t+1} ) | S_t=s ] 
						&= \sum_{a} π(a | s) \sum_{s',r} p (s', r | s, a ) [r+\gamma V_π (s' ) ]
						=> V_π &= R_π + \gamma P_πV_π	\tag{矩阵式}
						=> V_π &= (I - \gamma P_π)^{-1} R_π	\tag{解析解})
					$
					
					迭代计算数值解: $\lim_{k \to \infty} V_k = V_π$
					$
						V_{k+1}(s) & = \E_π [R_{t+1}+\gamma V_{k} (S_{t+1} ) | S_t=s ] 
						&= \sum_{a} π(a | s) \sum_{s' r} p (s', r | s, a ) [r+\gamma V_{k} (s') ]
					$
					
				* \bf{策略改进}: 找到累计奖励更多的策略$π'$.
					$\forall s \in S,\ V_π(s) \le Q(s, π'(s)) \quad => \quad V_π(s) \le V_{π'}(s)$
					贪心策略: $\forall s \in S,\ π' = \arg\max_a\ q_π (s,a)$
					
				* \bf{策略迭代} = 策略评估$V_π$ + 策略改进$π \to π'$
					$V_π => (π \to π') => V_π => (π \to π') => ... => \{π^*, V^*\}$
		...
		* 时序差分学习
			...
			\Example
				* \def{Q-Learning}
					--- off-policy
					$Δ Q(s,a) = \alpha\ ( R + \gamma · max\ Q(s',:) - Q(s,a))$

				* \def{Sarsa}
					--- on-policy
					$Δ Q(S_t,A_t) = \alpha\ ( R_{t+1} + \gamma\ Q(S_{t+1}, A_{t+1}) - Q(S_t, A_t))$
				...
				* Deep Q Networks
					...
					* 随机小批量更新
					...
					\bf{更新函数}: 神经网络(Q 网络)对于更新函数, 主要表现在参数$\theta$上
						...
						$J (\theta_t )=\underset{s, a \sim \rho(·)}{\E}  [ (y_t-Q (s, a ; \theta_t ) )^2 ]$
						$\nabla_{\theta_t} J (\theta_t )=\underset{s, a \sim \rho(·) \atop s' \sim \mathcal{E}}{\E} [ (r+\gamma \max _{a'} Q (s', a' ; \theta_{t-1} )-Q (s, a ; \theta_t ) ) \nabla_{\theta_t} Q (s, a ; \theta_t ) ]$
	...
	* 基于策略函数
		...
		* 策略梯度方法
			\def{策略梯度方法}
				是一种参数化策略的学习框架, 主要解决连续动作空间的强化学习问题,  通过最大化性能度量$J(\theta)$并梯度上升来更新$\theta$, 且 动作选择由策略函数决定, 而不再直接依赖价值函数, 无论是否同时学习一个近似的价值函数. 其中策略函数$π(a|s)$应当对$\theta$可导.

				\bf{策略梯度}, 
					$\P_π (a | s, \theta)$
					$ \theta_t = \theta_{t+1} + \alpha  \widehat{Δ J(\theta_t)}$
		...
		* 确定性策略梯度方法
			\def{确定性策略梯度方法}
				...
				\bf{确定策略梯度定理}: 采用状态-动作间的直接映射, 即确定性策略函数$a = \mu(s)$,  取代策略梯度中原有的随机性策略函数$\P_π(a|s)$, 原有函数估计给定状态下, 动作选择的概率. 确定性策略梯度, 是随机策略梯度的方差$\sigma \to 0$的极限情况, $\P_{π(\mu_\theta, \sigma \to 0)} \equiv \mu_\theta$. 
					$
						\nabla_\theta J (\mu_\theta) = .\int_{\mathcal{S}} \rho^{\mu}(s) \nabla_\theta \mu_\theta(s) \nabla_{a} Q^{\mu}(s, a) |_{a=\mu_\theta(s)}
						=\E_{s \sim \rho^{\mu}} [ .\nabla_\theta \mu_\theta(s) \nabla_{a} Q^{\mu}(s, a) |_{a=\mu_\theta(s)} ]
					$ 
					
					$\lim _{\sigma \to 0} \nabla_\theta J (π_{\mu_\theta, \sigma} )=\nabla_\theta J (\mu_\theta )$
			...
			$
				a = π(s) =  \{
				\underset{a}{\arg\max}\ Q_π(s, a)	& (\text{max})
