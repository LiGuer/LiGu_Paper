* 概率论
	* 概率空间
		\def
			...
			$\Omega$ 样本空间; $\P$, 概率; $\mathcal F$ 被选择的样本集合的集合, 且满足
					...
					\Property
						* 独立性 $ <=> \P(A B) = \P(A) \P(B)$. 即, $A, B$的发生互不影响.
						- 联合概率 - 条件概率关系
							...
							$
						* \Theorem{全概率公式}
							$\P(A) = \sum_i \P(A|B_i) \P(B_i) \quad; \sum_i A_i = \Omega$
						* \Theorem{Bayes公式}
							$\P(A | B)=\frac{\P(B | A) \P(A)}{\P(B)}$
							$\P(A_i | B)=\frac{\P(B | A_i) \P(A_i)}{\sum_j \P(B|A_j) \P(A_j)}; \sum_j A_j = \Omega$
				\Theorem{大数定律}
					* 弱大数定理
		...
		\Property
			...
			* 矩
				\def
					- K阶矩
						$
							E(X^k) &= \int_{-\infty}^{\infty} x^k f_X(x) \d x  \tag{连续式}
								&= \sum_i x_i^k \P_X(x_i)  \tag{离散式}
						$
					- K阶中心矩
						$
							E((X-\mu)^k) &= \int_{-\infty}^{\infty} (x-\mu)^k f_X(x) \d x  \tag{连续式}
								&= \sum_i (x_i-\mu)^k \P_X(x_i)  \tag{离散式}
						$
				* 联合矩
					\def
						$
					...
					\def
						$	
							E(X) &= \sum x_i \P_X(x_i)  \tag{离散式}
								&= \int_{-\infty}^{\infty} x f_X(x) \d x  \tag{连续式}
						$
						- 条件期望
					\Property
						$
							E(Y) &= \int_{-\infty}^{\infty} g(x) f_X(x) \d x
							Y &= g(X)  \tag{$g$是可测函数}
						$
		\Example
			* 离散概率分布
				...
				* Normal分布
					...
					* 多元Normal分布
						\Property
							...
							\Proof
								$
									D(x) &=\frac{1}{(2 π)^{\frac{D}{2}}} \frac{1}{|\.\Sigma|^{\frac{1}{2}}} \sum_{i=1}^D \sum_{j=1}^D \.u_i \.u_{j}^T \int e^{-\sum_{k=1}^D \frac{\.y_k^2}{2 λ_k}} y_i y_{j} \d \.y 
