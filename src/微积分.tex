\section 微积分
	\section 极限
		\section 数列极限
			\def{数列极限}{
				设$\{x_n\}$为一数列，如果存在常数a,对于任意给定的正数ε (不论它多么小)，总存在正整数N,使得当n>N时，不等式$\left|x_{n}-a\right|<\varepsilon$都成立，那么就称常数a是数列$\{x_n\}$的极限, 或者称数列$\{x_n\}$收敛于a,记为$\lim _{n \rightarrow \infty} x_{n}=a$. 若不存在常数a, 则数列无极限, 数列\bf{发散}.
...
\section 微分方程
    \section 解常微分方程组: Runge Kutta 方法
	*	[公式]:           ->   ->       ->      ->
...
\section 级数
	...
	*	[Example]:
	...
	\section Fast Fourier Transform 快速Fourier变换
	*	[定义]: 离散Fourier变换的高效算法
	*	[公式]:
